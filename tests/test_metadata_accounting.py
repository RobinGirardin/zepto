...
def test_parameter_trainable_defaults_true() -> None:
    ...
    metadata = TensorMetadata((8,), requires_grad=False)
...
def test_precision_policy_resolve_precedence() -> None:
    ...
    explicit = TensorMetadata((2,), dtype=DType.FP64)
    by_semantic = TensorMetadata((2,), semantic_type="weight")
    by_role = TensorMetadata((2,), role=TensorRole.GRADIENT)
    default = TensorMetadata((2,))
...
def test_accounting_policy_raises_on_unknown_resolved_dtype() -> None:
    ...
    policy = AccountingPolicy(PrecisionPolicy(default_dtype=DType.FP32))
    metadata = TensorMetadata((2, 3), dtype=DType.UNKNOWN)
