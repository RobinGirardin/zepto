...
from granite_validity import Catalog, FreeKnobs, derive_architecture
from granite_validity.identities import assert_identities, round_to_multiple


def test_derivation_matches_granite_family_core() -> None:
    from zepto.empirical.models.granite_core import GraniteFamilyCore
    ...
    family = GraniteFamilyCore().derive_options(
        {
            "num_kv_heads": 4,
            "gqa_group": 2,
            "head_dim": 128,
...
def test_large_vocab_is_off_by_default() -> None:
    catalog = Catalog()
    assert 100352 not in catalog.knobs.vocab_values()
    on = Catalog(knobs=type(catalog.knobs)(include_large_vocab=True, large_vocab=100352))
    assert 100352 in on.knobs.vocab_values()


def test_workload_respects_rope_window_and_token_cap() -> None:
    from granite_validity.catalog import WorkloadCatalog
