# Qwen3.8 text-only validity study

This directory is the executable Qwen3.8 instance of the protocol declared in
...
The estimand is the mean relative error on an **executable Qwen3.8-like hybrid
text grid**, not production 27B serving and not the vision tower.
...
Gated DeltaNet + SwiGLU is heavier than Apertus xIELU, so the default
128×9 = 1152-subject frame may OOM more often. `T_max` stays off until a
protocol-matched pilot. Vision and MTP are not scored.

## Layout

```text
python/qwen38_validity/    hybrid protocol + catalog + shims over validity_common
r/power.R                  wrapper; shared script is studies/validity_common/r/power.R
r/analysis.qmd             Qwen3.8 title + include of the shared TOST body
...
python -m qwen38_validity run \
  --n 12 --seed 1 \
  --out studies/qwen38-validity/artifacts/pilot

# 2. Lock n from the largest cell SD (needs R + TOSTER)
Rscript studies/validity_common/r/power.R \
  --evaluation studies/qwen38-validity/artifacts/pilot/evaluation.csv \
  --out studies/qwen38-validity/artifacts/pilot/sample_size.json
# or: Rscript studies/qwen38-validity/r/power.R (same args; sources the shared script)

# 3. Main sample (use n from sample_size.json)
python -m qwen38_validity run \
  --n N --seed 2 \
  --out studies/qwen38-validity/artifacts/main

# 4. Primary analysis
quarto render studies/qwen38-validity/r/analysis.qmd \
...
`studies/validity_common/python` then `studies/qwen38-validity/python`
...
- It does not download `Qwen/Qwen3.8-27B`. The HF twin is random-init
  `Qwen3_5ForCausalLM`.
- It does not score the vision tower or MTP.
