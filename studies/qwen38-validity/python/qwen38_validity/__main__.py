"""Command-line entry: python -m qwen38_validity ..."""
...
from qwen38_validity.collect import run_collection
from qwen38_validity.io import write_subjects
from qwen38_validity.protocol import OPTIONAL_LARGE_VOCAB
from qwen38_validity.sample import enumerate_frame, sample_subjects


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Qwen3.8 text-only validity study: sample subjects and collect measurements."
    )
...
def _add_shared(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--include-large-vocab",
        action="store_true",
        help="add vocab_size=248320 to the catalog",
    )
