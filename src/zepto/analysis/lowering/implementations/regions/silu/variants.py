...
from ....recipes.silu import DEFAULT_SILU_RECIPE, SiLURecipe
...
class FusedSiLURegionImplementation:
    """Lower ``Multiply(X, Sigmoid(X))`` as one fused SiLU region leaf."""

    descriptor: RegionImplementationDescriptor
    recipe: SiLURecipe = DEFAULT_SILU_RECIPE
    ...
    def compatible(
        ...
        if len(region.operation_ids) != 2:
            return "SiLU region expects two ops"
        families = tuple(
    ...
    def lower(
        ...
        *,
        ...
        if input_tensor is None or output_tensor is None:
            raise ValueError("SiLU region requires boundary tensors")
...
FUSED_SILU_REGION = FusedSiLURegionImplementation(
    descriptor=RegionImplementationDescriptor(
        id="region/silu",
        kind="region/silu",
        priority=10,
        capabilities=frozenset({"silu", "saved_input"}),
        provenance_rule=SILU_PROVENANCE,
        pattern_rule=SILU_PATTERN,
    ),
    recipe=DEFAULT_SILU_RECIPE,
    hardware_gate="any",
)

SILU_REGIONS: tuple[FusedSiLURegionImplementation, ...] = (FUSED_SILU_REGION,)
