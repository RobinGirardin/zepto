"""Fused linear + soft-cap + CE region lowering: reference, liger, hub-trl."""
...
def _requires_fused(context: InvocationContext) -> str | None:
    if "fused" not in context.requested_capabilities:
        return "linear_ce_softcap fusion requires requested_capabilities={'fused'}"
    return None
...
class FusedLinearCESoftcapRegionImplementation:
    """Lower decomposed LM-head linear + soft-cap + CE as one fused region leaf."""

    descriptor: RegionImplementationDescriptor
    recipe: LinearCESoftcapRecipe = DEFAULT_LINEAR_CE_SOFTCAP_RECIPE
    ...
    def weight_vocab_size(region: Region, graph: Graph) -> int:
        ...
        raise ValueError(
            "linear_ce_softcap region requires weight parameter on linear_matmul"
        )
...
def _descriptor(*, impl_id: str, priority: int) -> RegionImplementationDescriptor:
    return RegionImplementationDescriptor(
        ...
        provenance_rule=LINEAR_CE_SOFTCAP_PROVENANCE,
        pattern_rule=LINEAR_CE_SOFTCAP_PATTERN,
    )


FUSED_LINEAR_CE_SOFTCAP_REFERENCE = FusedLinearCESoftcapRegionImplementation(
    descriptor=_descriptor(
        impl_id="region/linear_ce_softcap/reference", priority=5
    ),
    recipe=DEFAULT_LINEAR_CE_SOFTCAP_RECIPE,
    hardware_gate="any",
)

FUSED_LINEAR_CE_SOFTCAP_HUB_TRL = FusedLinearCESoftcapRegionImplementation(
    descriptor=_descriptor(
        impl_id="region/linear_ce_softcap/hub-trl", priority=7
    ),
    recipe=DEFAULT_LINEAR_CE_SOFTCAP_RECIPE,
    hardware_gate="cuda_only",
)

FUSED_LINEAR_CE_SOFTCAP_LIGER = FusedLinearCESoftcapRegionImplementation(
    descriptor=_descriptor(
        impl_id="region/linear_ce_softcap/liger", priority=8
    ),
    recipe=DEFAULT_LINEAR_CE_SOFTCAP_RECIPE,
    hardware_gate="exclude_xpu_mps",
)

LINEAR_CE_SOFTCAP_REGIONS: tuple[FusedLinearCESoftcapRegionImplementation, ...] = (
