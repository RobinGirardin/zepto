...
def _score_requires_grad(estimation: RegionEstimationContext) -> bool:
    for tensor in estimation.input_tensors.values():
        if len(tensor.shape) >= 3 and tensor.requires_grad:
            return True
    return False


def _score_dims(tensor: Tensor) -> tuple[int, int, int]:
    """Return ``(batch, num_heads, seq_len)`` from rank-3 ``(h, S, S)`` scores."""
    shape = tensor.shape
    if len(shape) == 3:
    ...
    raise ValueError(
        f"masked_softmax region expects rank-3 (h, S, S) or rank-4 (B, h, S, S), "
        f"got {shape}"
    )
...
@dataclass(frozen=True, slots=True)
class FusedMaskedSoftmaxSinkRegionImplementation:
    """Lower scale+mask+sink softmax as one fused boundary-B region leaf."""

    descriptor: RegionImplementationDescriptor
    recipe: MaskedSoftmaxSinkRecipe = DEFAULT_MASKED_SOFTMAX_SINK_RECIPE
    hardware_gate: HardwareGate = "cuda_only"
    ...
    def compatible(
        ...
        if families not in MASKED_SOFTMAX_SINK_OP_SEQUENCES:
            return "unexpected operation sequence"
        if families[0] != "add":
            return "masked_softmax sink score path must start with add"
        if reason := _requires_fused(context):
            return reason
        return _check_hardware_gate(self.hardware_gate, context)
    ...
    def lower(
        ...
        *,
        ...
        if output_tensor is None and region.boundary_outputs:
            output_tensor = graph.edge(region.boundary_outputs[0]).tensor
        if input_tensor is None or output_tensor is None:
            raise ValueError("masked_softmax sink region requires boundary tensors")

        batch, num_heads, seq_len = _score_dims(output_tensor)
        output_id = region.boundary_outputs[0]
        lowered_out = edge_map[output_id]
        ...
        requires_grad = _score_requires_grad(estimation)
        if self.recipe.save_P and requires_grad:
            events.append(ResourceEvent(ResourceEventKind.SAVE, lowered_out))
...
def _sink_descriptor(*, impl_id: str, priority: int) -> RegionImplementationDescriptor:
    return RegionImplementationDescriptor(
        id=impl_id,
        kind="region/masked_softmax",
        priority=priority,
        capabilities=frozenset({"fused", "masked_softmax", "attention_sink"}),
        provenance_rule=None,
        pattern_rule=MASKED_SOFTMAX_SINK_PATTERN,
    )


FUSED_MASKED_SOFTMAX_SINK = FusedMaskedSoftmaxSinkRegionImplementation(
    descriptor=_sink_descriptor(
        impl_id="region/masked_softmax/sink", priority=9
    ),
    recipe=DEFAULT_MASKED_SOFTMAX_SINK_RECIPE,
    hardware_gate="cuda_only",
)

MASKED_SOFTMAX_REGIONS: tuple[
    FusedMaskedSoftmaxRegionImplementation | FusedMaskedSoftmaxSinkRegionImplementation,
    ...,
] = (
