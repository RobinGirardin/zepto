"""Fused SwiGLU region lowering: decomposed and Liger variants."""
...
def _requires_fused_gate_up(context: InvocationContext) -> str | None:
    if "fused_gate_up" not in context.requested_capabilities:
        return "liger-fused-gate-up requires requested_capabilities={'fused_gate_up'}"
    return None
...
def _extract_dims(
    ...
    if len(input_tensor.shape) != 2:
        raise ValueError("SwiGLU region expects rank-2 input (S, d)")
...
class FusedSwiGLURegionImplementation:
    """Lower a SwiGLU MLP stack as one fused region leaf."""

    descriptor: RegionImplementationDescriptor
    recipe: SwiGLURecipe = DEFAULT_SWIGLU_RECIPE
    hardware_gate: HardwareGate = "any"
    elide_silu_temp: bool = False
    ...
    def compatible(
        ...
        if anchor_type is not None and anchor_type != "SwiGLU":
            return "not a SwiGLU module invocation"
        if len(region.operation_ids) != 6:
            return "SwiGLU fusion expects six ops"
        families = tuple(
        ...
        if self.capability_check == "fused_post_gemm":
            if reason := _requires_fused_post_gemm(context):
                return reason
        elif self.capability_check == "fused_gate_up":
            if reason := _requires_fused_gate_up(context):
                return reason
    ...
    def lower(
        ...
        *,
        ...
        if input_tensor is None:
            raise ValueError("SwiGLU region requires boundary input tensor")
        ...
        if self.recipe.save_fused_gate_up:
            gate_up_id = f"region:{region.id}:gate_up"
            register_auxiliary_edge(
                gate_up_id,
                Tensor(
                    shape=(seq_len, 2 * intermediate_size),
                    semantic_type="swiglu_gate_up_fused",
                    requires_grad=input_tensor.requires_grad,
                    persistent=False,
                ),
                role_ctx=RoleContext(explicit_role=TensorRole.AUXILIARY),
                context=context,
                storage_id=gate_up_id,
                lowered_edges=lowered_edges,
            )
            events.append(ResourceEvent(ResourceEventKind.ALLOCATE, gate_up_id))
            if input_tensor.requires_grad:
                events.append(ResourceEvent(ResourceEventKind.SAVE, gate_up_id))
                release_edges.append(gate_up_id)
            auxiliary_edges.append(gate_up_id)
        else:
        ...
        if not self.elide_silu_temp:
            silu_id = f"region:{region.id}:silu_g"
            register_auxiliary_edge(
                silu_id,
                Tensor(
                    shape=(seq_len, intermediate_size),
                    semantic_type="swiglu_silu_g",
                    requires_grad=input_tensor.requires_grad,
                    persistent=False,
                ),
                role_ctx=RoleContext(explicit_role=TensorRole.AUXILIARY),
            ...
            events.append(ResourceEvent(ResourceEventKind.ALLOCATE, silu_id))
            auxiliary_edges.append(silu_id)
...
def _descriptor(
    *,
    ...
    return RegionImplementationDescriptor(
        ...
        provenance_rule=SWIGLU_PROVENANCE,
        pattern_rule=SWIGLU_PATTERN,
    )


FUSED_SWIGLU_DECOMPOSED = FusedSwiGLURegionImplementation(
    ...
    hardware_gate="any",
    elide_silu_temp=False,
)

FUSED_SWIGLU_LIGER = FusedSwiGLURegionImplementation(
    descriptor=_descriptor(
        impl_id="region/swiglu/liger",
        priority=10,
        capabilities=frozenset({"swiglu", "fused_post_gemm"}),
    ),
    recipe=LIGER_SWIGLU_RECIPE,
    hardware_gate="cuda_only",
    elide_silu_temp=True,
    capability_check="fused_post_gemm",
)

FUSED_SWIGLU_LIGER_FUSED_GATE_UP = FusedSwiGLURegionImplementation(
    descriptor=_descriptor(
        impl_id="region/swiglu/liger-fused-gate-up",
        priority=9,
        capabilities=frozenset({"swiglu", "fused_gate_up"}),
    ),
    recipe=LIGER_FUSED_GATE_UP_SWIGLU_RECIPE,
    hardware_gate="cuda_only",
    elide_silu_temp=True,
    capability_check="fused_gate_up",
)

SWIGLU_REGIONS: tuple[FusedSwiGLURegionImplementation, ...] = (
