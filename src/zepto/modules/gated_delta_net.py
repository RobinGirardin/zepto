...
class GatedDeltaNet(Module):
    ...
    def forward(
        ...
        b, a = _split_last(ba, (cfg.num_v_heads, cfg.num_v_heads))

        qkv = Concat(axis=1, input_count=3)(q, k, v)  # type: ignore[call-arg]
        conv_out, conv_state_out = self.conv(qkv, conv_state_in)
        q_c, k_c, v_c = _split_last(
            conv_out,
        ...
        neg_decay = Multiply()(scale_by_parameter(sp, self.decay_rate), self._minus_one)  # type: ignore[call-arg]
        ...
        scan_heads = Reshape(shape=(seq_len, cfg.num_v_heads, cfg.head_dim))(scan_out)
        ...
        for scan_h, z_h in zip(scan_splits, z_splits, strict=True):
            val = Reshape(shape=(seq_len, cfg.head_dim))(scan_h)
            gate_h = Reshape(shape=(seq_len, cfg.head_dim))(z_h)
            normed_heads.append(self.out_norm(val, gate_h))
        merged = Concat(axis=1, input_count=len(normed_heads))(*normed_heads)  # type: ignore[call-arg]
