...
from zepto.modules._internal._helpers import RMSNORM_COMPUTE_DTYPE, activation_dtype
...
class GatedDeltaScan(Module):
    ...
    def forward(
        ...
        if batch == 1:
            ...
            q_parts = split_on_sequence(query, seq_len)
            k_parts = split_on_sequence(key, seq_len)
            v_parts = split_on_sequence(value, seq_len)
            beta_parts = split_on_sequence(beta, seq_len)
            decay_parts = split_on_sequence(log_decay, seq_len)
        ...
        for _t in range(seq_len):
            if batch == 1:
                ...
                beta_t = Reshape(shape=(num_heads, 1))(beta_parts[_t])
                decay_t = Reshape(shape=(num_heads, 1, 1))(decay_parts[_t])
            else:
                q_t = Reshape(shape=(batch, num_heads, key_dim))(q_parts[_t])
                k_t = Reshape(shape=(batch, num_heads, key_dim))(k_parts[_t])
                v_t = Reshape(shape=(batch, num_heads, self.value_head_dim))(
                    v_parts[_t]
                )
                beta_t = Reshape(shape=(batch, num_heads, 1))(beta_parts[_t])
                decay_t = Reshape(shape=(batch, num_heads, 1, 1))(decay_parts[_t])
            ...
            if batch == 1:
                ...
                outputs.append(
                    Reshape(shape=(1, num_heads, self.value_head_dim))(out_t)
                )
            else:
                k_row = Reshape(shape=(batch, num_heads, 1, key_dim))(k_t)
                prediction = MatMul()(k_row, decayed)  # type: ignore[call-arg]
                prediction = Reshape(shape=(batch, num_heads, self.value_head_dim))(
                    prediction
                )
                delta = Multiply()(beta_t, Subtract()(v_t, prediction))  # type: ignore[call-arg]
                k_col = Reshape(shape=(batch, num_heads, key_dim, 1))(k_t)
                delta_row = Reshape(
                    shape=(batch, num_heads, 1, self.value_head_dim)
                )(delta)
                outer = MatMul()(k_col, delta_row)  # type: ignore[call-arg]
                state = Add()(decayed, outer)  # type: ignore[assignment]
                q_row = Reshape(shape=(batch, num_heads, 1, key_dim))(q_t)
                out_t = MatMul()(q_row, state)  # type: ignore[call-arg]
                outputs.append(
