"""Nemotron routed ReLU² expert pool."""
...
from zepto.modules.moe.moe_routing import UniformRoutingProfile, zero_accumulator_like
from zepto.modules.ffn.squared_relu_ffn import SquaredReluFFN


class NemotronExpertPool(Module):
    """Unrolled SquaredReluFFN experts with gather/scatter dispatch."""

    module_kind = "NemotronExpertPool"
        ...
        *,
        ...
        for expert_id in range(num_experts):
            setattr(
                self,
                f"expert_{expert_id}",
                SquaredReluFFN(hidden_size, intermediate_size),
            )
        if expert_indices is not None:
            ...
            from zepto.modules.moe.moe_routing import synthesize_expert_indices
    ...
    def forward(
        ...
        for expert_id in range(len(self._expert_indices)):
